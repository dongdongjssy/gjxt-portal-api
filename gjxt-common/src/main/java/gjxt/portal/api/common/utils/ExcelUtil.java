...
import gjxt.portal.api.common.annotation.Excel;
...
public class ExcelUtil<T>
{
     ...
     * @return 结果
     */
    public String exportExcel(List<T> list, String sheetName, String downloadPath)
    {
        OutputStream out = null;
        HSSFWorkbook workbook = null;
        try
...
//            return AjaxResult.success(filename);
     ...
     * @return 结果
     */
    public String exportExcelMap(List<Map<String, Object>> list, String sheetName, String downloadPath)
    {
        OutputStream out = null;
        HSSFWorkbook workbook = null;
        try
        {
            // 得到所有定义字段
            // 得到所有field并存放到一个list中.
            List<String> fields = new ArrayList<>(list.get(0).keySet());
            ...
            for (int index = 0; index <= sheetNo; index++)
            {
                ...
                for (int i = startNo; i < endNo; i++)
                {
                    ...
                    for (int j = 0; j < fields.size(); j++)
                    {
                        ...
                        cell.setCellType(CellType.STRING);
                        // 如果数据存在就填入,不存在填入空格.
                        cell.setCellValue(vo.get(field).toString());

                    }
                }
            }

            String filename = DateUtils.dateTimeNow()+sheetName;
            out = new FileOutputStream(getAbsoluteFile(downloadPath, filename));
            workbook.write(out);
//            return AjaxResult.success(filename);
            return filename;
...
//        filename = LocalDate.now().toString().replace("-", "") + "_" + filename + ".xls";
        return filename;
    }
     ...
     * @param filename 文件名称
     */
    public String getAbsoluteFile(String path, String filename)
    {
